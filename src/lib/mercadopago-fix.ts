...
export class MercadoPagoService {
   ...
   * 🆕 FIX: Mejora las URLs de retorno y agrega más validaciones
   */
  private async buildPreferenceRequest(checkoutData: CheckoutData): Promise<PreferenceRequest> {
    ...
    // 🆕 FIX: Mejorar las URLs de retorno con parámetros adicionales
    ...
    const preference: PreferenceRequest = {
      ...
      payer: {
        ...
        address: checkoutData.shippingAddress ? {
          ...
          street_number: parseInt(checkoutData.shippingAddress.number) || 0,
   ...
   * 🆕 FIX MEJORADO: Añade retry y mejor manejo de errores
   */
  redirectToCheckout(preference: PreferenceResponse): void {
    ...
    // 🆕 FIX: Añadir pequeño delay antes de redirigir
    console.log('⏳ Esperando 500ms antes de redirigir...');
    setTimeout(() => {
      console.log('➡️ Redirigiendo a:', checkoutUrl);
      
      // 🆕 FIX: Usar window.location.assign en lugar de href para mejor compatibilidad
      try {
